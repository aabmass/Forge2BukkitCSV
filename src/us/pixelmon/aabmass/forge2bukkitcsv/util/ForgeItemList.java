...
/**
 * A big thanks to ForgeEssentials for this code!
 * It comes from their FriendlyItemsList.java, but
 * I added some obfuscations so I wouldn't have to
			...
			if (item != null) {
				try {
					itemMap.put(item.func_77658_a().toLowerCase()
							.replaceAll("tile.", "").replaceAll("item.", ""),
							item);
				}
				catch (Exception e) {
			...
			if (block != null && block.field_71990_ca != 0) {
				try {
					...
					blockMap.put(block.func_71917_a().toLowerCase()
							.replaceAll("item.", "").replaceAll("tile.", ""),
							block);
				}
				catch (Exception e) {
